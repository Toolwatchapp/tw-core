...
})(this, function(__WEBPACK_EXTERNAL_MODULE_0__, __WEBPACK_EXTERNAL_MODULE_5__, __WEBPACK_EXTERNAL_MODULE_13__, __WEBPACK_EXTERNAL_MODULE_24__, __WEBPACK_EXTERNAL_MODULE_25__) {
...
var TwAPIService = TwAPIService_1 = (function () {
    ...
    TwAPIService.prototype.getBrands = function () {
        return this.http.get(TwAPIService_1.assetsUrl + '/watches/watch-brand.json')
            .map(function (res) { return res.json(); })
            .toPromise().then(function (brands) {
    ...
    TwAPIService.prototype.getModels = function (brand) {
        ...
        brand = __WEBPACK_IMPORTED_MODULE_5__helpers_string_helper__["a" /* StringHelper */].replaceAll(brand, "&", "");
        return this.http.get(TwAPIService_1.assetsUrl + '/watches/watch-models/' + brand + ".json")
            .map(function (res) { return res.json(); })
    ...
    TwAPIService.prototype.getCalibers = function (brand) {
        brand = __WEBPACK_IMPORTED_MODULE_5__helpers_string_helper__["a" /* StringHelper */].replaceAll(brand, " ", "");
        brand = __WEBPACK_IMPORTED_MODULE_5__helpers_string_helper__["a" /* StringHelper */].replaceAll(brand, "&", "");
        return this.http.get(TwAPIService_1.assetsUrl + '/watches/watch-calibers/' + brand + ".json")
            .map(function (res) { return res.json(); })
    ...
    TwAPIService.prototype.accurateTime = function (statusCallback, precison) {
        ...
        if (TwAPIService_1.time === undefined ||
            now() - TwAPIService_1.time.syncAnchor > 2 * 60 * 1000) {
            ...
            return Promise.all(promises).then(function (results) {
                ...
                results = results.slice(Math.round(results.length * 0.1), results.length - 1 - Math.round(results.length * 0.1));
                //Compute average
                var sum = results = results.reduce(function (previous, current) { return current += previous; });
                var averageOffset = sum / results.length;
                TwAPIService_1.time = {
            ...
            TwAPIService_1.time.syncDate = new Date(TwAPIService_1.time.syncDate.getTime() +
                now() - TwAPIService_1.time.syncAnchor);
            TwAPIService_1.time.syncAnchor = now();
